...
def write_video_info(connection, video_response, publish_time):
    ...
    insert_query = """INSERT INTO social_network_post
            (attachment, likes, comments, reposts, content) VALUES (FALSE, {}, {}, 0, '{}') RETURNING id;""".format(
        like_count, comment_count, title + "\n" + description)
    ...
    insert_query = (
        """INSERT INTO data(post_id, CT, ER, CP, CP_MIN) VALUES ({}, {}, {}, {}, {})""".format(id_video, ct, er, cp,
                                                                                               cp_min)
...
def calculating_engagement_rate(number_post_views, number_subscribers):
    return round(int(number_post_views) / int(number_subscribers) * 100, 2)
...
def calculating_post_profit(number_post_views, likes_count, dislikes_count, video_length, comments_count):
    ...
    return round(reach1 * t1 * cps + reach2 * (t2 - t1) * ke * cps, 2)
